...
class RESTTest(unittest.TestCase):
    ...
    def test_generate_complete_graphs(self):
        res = get('%s/api/Experiment' % base).json()
        token3 = res['token']
        self.assertIsNotNone(token3)
        ...
        for e in endpoints:
            if e['name'] == 'CompleteGraph':
                gr = put("%s%s" % (base, e['uri']), data={'n': 40, 'token': token3})
                self.assertEqual(gr.status_code, 200)

        res = delete('%s/api/Experiment' % base, data={'token': token3})
        self.assertEqual(res.status_code, 200)

        print "Generate CompleteGraph: OK"
    ...
    def test_load_experiments(self):
        ...
        for e in endpoints:
            if e['name'] == 'BAGraph':
                gr = put("%s%s" % (base, e['uri']), data={'n': 2000, 'm': 10, 'token': token})
                self.assertEqual(gr.status_code, 200)
        ...
        res = put('%s/api/Voter' % base, data={'infected': 0.1, 'token': token})
        self.assertEqual(res.status_code, 200)
        print "Load Voter: OK"

        res = put('%s/api/QVoter' % base, data={'infected': 0.1, 'q': 5, 'token': token})
        self.assertEqual(res.status_code, 200)
        print "Load QVoter: OK"

        res = put('%s/api/MajorityRule' % base, data={'infected': 0.1, 'q': 5, 'token': token})
        self.assertEqual(res.status_code, 200)
        print "Load MajorityRule: OK"

        res = put('%s/api/Sznajd' % base, data={'infected': 0.1, 'token': token})
        self.assertEqual(res.status_code, 200)
    ...
    def test_upload_graph(self):
        ...
        jd = json.dumps(js)
        res = put('%s/api/UploadNetwork' % base, data={'file': str(jd), 'directed': False, 'token': token})
        self.assertEqual(res.status_code, 200)
        print "Upload Network: OK"
